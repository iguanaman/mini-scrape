...
Personal tool for browsing miniature wargaming products by manufacturer range, with retailer prices. Home page shows manufacturer sections with range pills — clicking a range expands inline product cards sorted by price-per-mini. Cards show a price sticker, wishlist heart, hide eye, owned counter, and minis counter. Clicking a card image opens a lightbox. Store-filter icons in the header filter which cards are visible (offer rows and sticker always show all stores); each icon cycles off → selected → lowest-only (gold), where gold shows only cards on which that retailer is the cheapest of all stores.
